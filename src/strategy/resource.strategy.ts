...
function rememberNewEnergySource(creep: Creep, source: Storage) {
  creep.memory.energySourceId = source ? source.id : undefined;
}


export function rememberOldEnergySource(creep: Creep) {
  return Game.getObjectById<Storage>(creep.memory.energySourceId);
}

export function harvestNearestEnergySource(creep: Creep) {
  let result = -20;
...
export function harvestFixedEnergySource(creep: Creep) {
  let result = -20;
  if (shouldHarvestEnergy(creep)) {
    creep.memory.task = GATHER_ENERGY;
    let energySource = rememberOldEnergySource(creep);
    if (!energySource || energySource.storage) {
      ...
      if (Object.keys(sourcePlan).length > 0) {
        ...
        if (unassignedIdx !== -1) {
          ...
          plan.push(creep.id);
          // rememberNewEnergySource(creep, { id: sourceId } as any); // FIXME: wtf?
...
export function harvestEnergyFromNearestContainer(creep) {
  let result = -20;
  if (shouldHarvestEnergy(creep)) {
    creep.memory.task = GATHER_ENERGY;
    let energySource = rememberOldEnergySource(creep);
...
export function harvestEnergyFromNearestStorage(creep) {
  let result = -20;
  if (shouldHarvestEnergy(creep)) {
    creep.memory.task = GATHER_ENERGY;
    let energySource = rememberOldEnergySource(creep);
